...
public:
    ...
    void build_mesh(const aiMesh* mesh, uint subdiv_level) noexcept {
        ...
        if (ai_normals) { _properties |= Shape::property_flag_has_vertex_normal; }
        if (ai_uvs) { _properties |= Shape::property_flag_has_vertex_uv; }
    ...
    [[nodiscard]] static auto load(
        std::filesystem::path path, uint subdiv_level,
    ...
    [[nodiscard]] static auto load(
        ...
        auto future = global_thread_pool().async([&] {
            MeshLoader loader;
            loader.build_mesh(positions, triangles, normals, uvs);
            return loader;
        });
        return future;
    }
