...
public:
    ...
    SphereGroup(Scene *scene, const RawSpheresInfo &spheres_info) noexcept
        : Shape{scene, spheres_info.shape_info}
    {
        auto sphere_mesh = SphereGeometry::create(spheres_info.subdiv).get().mesh();
        auto vertex_count = sphere_mesh.vertices.size();
        auto triangle_count = sphere_mesh.triangles.size();

        auto centers = spheres_info.centers;
        ...
        for (auto i = 0u; i < center_count; ++i) {
            auto center = make_float3(centers[i * 3u + 0u], centers[i * 3u + 1u], centers[i * 3u + 2u]);
            for (auto v: sphere_mesh.vertices) {
                auto vertex = Vertex::encode(
                    p.position() + center, p.normal(), p.uv
                );
...
LUISA_EXPORT_API luisa::render::SceneNode *create_raw(
    ...
    const luisa::render::RawSpheresInfo &spheres_info) LUISA_NOEXCEPT {
    return luisa::new_with_allocator<luisa::render::SphereGroupWrapper>(scene, spheres_info);
}